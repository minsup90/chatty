...
public class BottomSheet extends Dialog {
    ...
    private static class BottomSheetCell extends FrameLayout {
        ...
        public BottomSheetCell(Context context, boolean grid) {
            ...
            if (grid) {
                addView(imageView, LayoutHelper.createFrame(48, 48, Gravity.CENTER_HORIZONTAL | Gravity.TOP, 0, 0, 0, 0));
            } else {
...
//                textView.setMaxHeight(textView.getHeight() / 2);

            if (grid) {
                textView.setTextColor(0xff757575);
                textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 12);
                addView(textView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP, 0, 0, 0, 0));
            } else {
            ...
            for (int a = 0; a < items.length; a++) {
                ...
                if (isGrid) {
                    ...
                    if (rowLayout == null || lastRowLayoutNum != row) {
                        ...
                        containerView.addView(rowLayout, LayoutHelper.createLinear(LayoutHelper.MATCH_PARENT, 80, 0, lastRowLayoutNum != 0 ? 0 : 0, 0, 0));
